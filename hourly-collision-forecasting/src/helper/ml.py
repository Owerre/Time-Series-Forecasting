...
class RegressionModels:
    ...
    def custom_scorer(self):
        """
        Define custom scorer currently not available in sklearn scoring list.
    ...
    def evaluate_metric_cv(self, model, X_train, y_train, cv_fold, model_nm=None):
        """
        ...
        errors = {'wMAPE = {}'.format(np.round(wmape.mean(),3)), 
    ...
    def plot_wmape_svr(self, X_train, y_train, cv_fold):
        ...
        mae_list = [pd.Series(0.0, index = range(len(C_list))) 
                    for _ in range(len(gamma_list))
                   ]
        wmape_list = [pd.Series(0.0, index = range(len(C_list))) 
                     for _ in range(len(gamma_list))
                    ]
        ...
        for i, val1 in enumerate(gamma_list):
            ...
            wmape_list[i].plot(label = "gamma="+str(gamma_labels[i]), marker = "o", 
                              linestyle = "-", ax = ax1
                             )
            mae_list[i].plot(label = "gamma="+str(gamma_labels[i]), marker = "o", 
                             linestyle = "-", ax = ax2
                            )

        ax1.set_xlabel("C", fontsize = 15)
        ax1.set_ylabel("wMAPE", fontsize = 15)
        ax1.set_title("5-Fold Cross-Validation with RBF kernel SVR", fontsize = 15)
        ...
        ax2.set_title("5-Fold Cross-Validation with RBF kernel SVR", fontsize = 15)
    ...
    def diagnostic_plot(self, y_true, y_pred, marker=None, color=None, label=None):
        """
        Diagnostics plot.
        ...
        # plot figures
        plt.subplot(221)
        plt.plot(range(1,len(rstandard)+1), rstandard, color=color, marker=marker,
        ...
        plt.plot(np.arange(1,len(lb)+1), lb, color=color, marker=marker,
                 markerfacecolor='none', linestyle='', label=label)
        ...
        plt.subplot(224)
        plt.acorr(residual, usevlines=True, maxlags=49, 
        normed=True, color=color)
        plt.acorr(residual, usevlines=False, maxlags=49, 
        normed=True, color=color, label=label)
        plt.axhline(y=1.96/np.sqrt(len(residual)),linestyle= '--', 
    ...
    def diagnostic_plot_(self, y_pred, y_true, ylim=None):
        ...
        ax2.text(0.3, 0.1, 
        r'$R^2 = {},~ wMAPE = {}$'.format(str(r2), str(wmape)), fontsize=20)
    ...
    def error_metrics(self, y_true, y_pred):
        ...
        errors = {'WMAPE = {}'.format(np.round(wmape,3)), 
